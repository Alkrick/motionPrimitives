 ...
 * @brief  My implementation of Probablistic Motion Primitives (ProMP) based on
 * 2013 Paper by Alexandros Paraschos et. al
 * @version 0.1
 * @date 2022-12-10
 *
...
public:
  ...
  /**
   * @brief Combines two Motion Primitives
   * It produced a motion primitive whose probability distribution
   * covers the overlapping regions between the input primitives
   *
   * @param mp1
   * @param mp2
   * @return ProMP
   */
  static ProMP combine(ProMP &mp1, ProMP &mp2);

  /**
   * @brief Blends two motion primitives based on activation function
   * that determines each primitives activation period
   *
   * @param mp1
   * @param mp2
   ...
   * @return ProMP
   */
  static ProMP blend(ProMP &mp1, ProMP &mp2, Eigen::VectorXd &a);
...
private:
  ...
  /**
   * @brief Learn average weights of basis functions from demonstration
   * trajectories
   *
  ...
  Eigen::VectorXd sigmoid(const Eigen::VectorXd& t,double x);
};
...
public:
   ...
   */
  Eigen::VectorXd getPhase() { return z_; }
